...
<script>
// ── ذخیره نتیجه ریسک در IDB (آنلاین و آفلاین) ──────────────────────────
async function saveRiskToIDB(scoreData) {
  const entry = {
    id: Date.now(),
    assessment_date: new Date().toISOString().split('T')[0],
    risk_score:      scoreData.score,
    risk_level:      scoreData.level,
    risk_probability: scoreData.prob,
    population_prev: scoreData.popPrev,
    relative_risk:   scoreData.relRisk,
    factors:         JSON.stringify(scoreData.factors || {}),
    recommendations: scoreData.recommendations || '',
    _cached:         Date.now(),
  };
  if (typeof PakDavaDB !== 'undefined') {
    await PakDavaDB.set('risk_records', entry);
  }
  if (typeof PakDavaSync !== 'undefined') {
    await PakDavaSync.enqueue('risk_queue', {
      data: { ...entry, score: scoreData.score }
    });
  }
}
window.saveRiskToIDB = saveRiskToIDB;
</script>
</body>
</html>